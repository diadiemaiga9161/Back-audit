...
//               signUpRequest.getAdresse(),
       ...
       if (strRoles == null) {
           Role resPonRole = roleRepository.findByName(ERole.ROLE_ENTREPRISE)
           ...
           strRoles.forEach(role -> {
               switch (role) {
                   ...
                   case "superadmin":
                       ...
                       break;
                   case "entreprise":
                       Role entreprise = roleRepository.findByName(ERole.ROLE_ENTREPRISE)
                               .orElseThrow(() -> new RuntimeException("Error: Le rôle entreprise n'a pas été trouvé."));
                       roles.add(entreprise);
                       break;

                   default:
                       Role auditeur = roleRepository.findByName(ERole.ROLE_AUDITEUR)
